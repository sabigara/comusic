...
function padStr(numStr: string, pad: string, length: number, right: boolean = false) {
  if (right) {
...
const Clock: React.FC = () => {
  ...
  return (
    <Wrapper>
      <Mins><TimeLabel>{padStr(minutes.toString(), '0', 2)}</TimeLabel></Mins>
      <Colon><TimeLabel>:</TimeLabel></Colon>
      <Secs><TimeLabel>{padStr(seconds.toString(), '0', 2)}</TimeLabel></Secs>
      <TimeLabel>.</TimeLabel>
      <MiliSecs><MiliSecLabel>{padStr(strMiliSecs, '0', 2, true)}</MiliSecLabel></MiliSecs>
    </Wrapper>
  )
}
